...
const EditModal: React.FC<EditModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (type === 'book') {
      ...
      if (authors.length > 0) {
        ...
        if (allowNewAuthor && useNewAuthor && !formData.new_author_name?.trim()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'book' && authors.length > 0 && allowNewAuthor && useNewAuthor) {
